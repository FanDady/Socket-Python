...
# 这里在云服务器端直接集成了MQTT客户端和Socket接收端，即当视频传输到云服务器并接收到后的时候云服务器同时
# 也直接作为MQTT的客户端向topic发送信息，这样做的理由主要是考虑到当检测到目标时录制视频需要一些时间，如果
# 由工控机发送消息那么信息会比视频文件先到这样程序上不能马上显示文件，因此直接在云服务器端集成做到文件接收
# 的时候才发送信息
